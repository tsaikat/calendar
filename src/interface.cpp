...
void Interface::printHeader() const {
    clearConsole();
    std::cout << std::setfill(' ') << std::setw(22) << "Date And Time |"
              << std::setfill(' ') << std::setw(30) << "Name of Event |"
              << std::setfill(' ') << std::setw(22) <<      "Location |"
              << std::setfill(' ') << std::setw(20) <<  "Participants |"
              << std::setfill(' ') << std::setw(28) << "Type of Event |"
              << std::setfill(' ') << std::setw(18) <<      "Duration |" <<std::endl;
...
std::string Interface::promptStr(const std::string& question, const size_t length) const {
    std::string answer;
    while ( answer.empty()) {
        std::cout << question <<std::endl;
        ...
        for (auto& it : answer) {
            if (it == ',') {
                std::cout << "Please try without comma. Comma is not allowed"<< std::endl;
                answer.clear();
                break;
            }
        }
...
void Interface::addEvent() {
    ...
    eventName = promptStr("Type your event name:");
    eventPlace = promptStr("Type your event location:");
    ...
    if (eventType == 1) {
        std::string eventReason[3] = {"Hosting", "Presenting", "Attending"};
        int i = promptChoice("Choose one - Are you 1.Hosting/2.Presenting/3.Attending this the event?", 1, 3);
        newEvent = std::make_shared<Important>(eventName, eventPlace, eventDuration, eventParticipants, eventReason[i-1]);
    } else if (eventType == 2) {
        bool lessVis = promptBool("Do you want to make this event less visibile on the calendar? (y/n)", "y", "n");
        newEvent = std::make_shared<Optional>(eventName, eventPlace, eventDuration, eventParticipants, lessVis);
    } else if (eventType == 3) {
        std::cout << "Untill when the event can be moved?" << std::endl;
        DateTime moveUnitll = promptDateTime();
        newEvent = std::make_shared<Moveable>(eventName, eventPlace, eventDuration, eventParticipants, moveUnitll);
    }
...
void Interface::findEvent () {
    ...
    while (true) {
        ...
        if ( choice == 0) {
            ...
            std::string name = promptStr("Type the name of the event you're looking for:");
            clearConsole();
            printHeader();
           ...
           std::string name = promptStr("Type the location of the event you're looking for:");
            clearConsole();
            printHeader();
...
void Interface::deleteEvent() {
        ...
        DateTime date;
        date = promptDateTime(true); // set to true because we need date too
...
void Interface::monthlyView() {
    ...
    for (size_t i = 1; i <= date.daysInMonth(); ++i) {
        ...
        DateTime dayEnd = date;
        dayEnd.setTime(23);
        // print all dates in the month here
        std::cout << std::setfill (' ') << std::setw (22) << dayBegin.formatDate();
        if (!calendar.printEvents(dayBegin, dayEnd) ) std::cout << std::endl;
    }
    goBack();
